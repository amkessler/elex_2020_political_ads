...
####
The following summaries are compiled from Ad Analytics' database of all political ads that have aired on television in U.S. markets nationwide. Here we're looking only at the ads associated with the two Georgia Senate runoffs that could determine the balance of power in the U.S. Senate. The runoff elections will take place on Jan. 5.  
...
#
# Contracts for Future Spending

Ad Analytics also captures contracts signed with TV stations for future spending on political ads. The numbers below reflect the currently available data for **airtime booked for after `r format(Sys.time(), '%b. %d')`**.  
  
(Note that these are *booking contracts only* and may change, and they frequently do in fast-moving campaigns.)

```{r, echo=FALSE}

#limit to only future spending
...
ga_runoff_delta %>% 
  ...
  fmt_currency(
   ...
   tab_header(
    title = md("Future Spending by **Candidate**"),
    subtitle = "(Contracts Only)"
  )
